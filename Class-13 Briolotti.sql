...
Exercise 1
Add a new customer
To store 1, For address use an existing address.
The one that has the biggest address_id in 'United States'
*/
...
SELECT * FROM customer WHERE last_name = "Briolotti";
...
ON UPDATE CASCADE)
The solution for this is to first delete (in order from child to parent) the rows that the movie is related to.
You can also disable FOREIGN KEY CHECK and then enable it again, but this is not recommended
*/
DELETE FROM payment
WHERE rental_id IN (
        SELECT rental_id
